...
public class GenerateCarnivalistPerOccurrenceInEmpire : SpecialBehaviour
{
    ...
    public override void InitializeSpecialBehaviour(Tile behaviourTile)
    {
        foreach (Tile tile in ExploitationManager.Instance.Infrastructures)
        {
            if (tile.TileData is InfrastructureData data && _tilesBoosting.Contains(data))
            {
                ...
                behaviourTile.UpdateCarnivalists(_carnivalistQuantity, tile);
            }
        }
    ...
    {
        return $"Recruits {_carnivalistQuantity}<sprite name=\"Carnivalist_Emoji\"> for each occurrence of {_tilesBoosting.ToCustomString(true, false, false)} in the empire";
    }
}
